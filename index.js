...
async function main() {
    ...
    while (currentDate.toISOString().split('T')[0] <= endDate) {
        ...
        const mediaData = await fetchMediaData(headers, DEPENDENT_ID, formattedDate, formattedDate);

        if (mediaData.length === 0) {
            console.log(`No media found for date: ${formattedDate}`);
        ...
        for (const media of mediaData) {
            ...
            const monthFolderPath = getMonthFolderPath(outputDir, media.for_date);
